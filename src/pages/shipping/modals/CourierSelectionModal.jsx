...
const CourierSelectionModal = ({ isOpen, order, onClose, onShipmentCreated, onAssignCourier, onGeneratePickup }) => {
    ...
    const courierAlreadyAssigned = !!(order?.courier_company_id || order?.courier_company_name)
    ...
    useEffect(() => {
        ...
        if (courierAlreadyAssigned) {
            getShippingLocationById(order?.pickup_location_id)
            fetchAvailableCouriers()
            setStep(4)
        } else if (selectedLocation && userAddress) {
            fetchAvailableCouriers()
            setStep(3)
        }
    }, [shipmentId, courierAlreadyAssigned, selectedLocation, userAddress])
    ...
    const getShippingLocationById = async (id) => {
        try {
            const response = await ordersAPI.getPickupLocationById(id)
            if (response.data.success) {
    ...
    const shipmentCreated = async () => {
        await getShippingLocationById(order?.pickup_location_id)
    ...
    const fetchAvailableCouriers = async () => {
        ...
        try {
            ...
            if (response.data.data.success && response.data.data.availableCouriers) {
                setAvailableCouriers(response.data.data.availableCouriers)
            } else {
    ...
    const handleCreateShipment = async () => {
        ...
        try {
            ...
            const shipmentData = {
                ...
                pickupLocation: locationData?.location_name,
            ...
            if (response.data.success) {
                ...
                setShipmentId(response.data.data.shipmentId)
                setStep(3)
            ...
            <div className="bg-white rounded-lg w-full max-w-4xl mx-4 max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-6">
                    ...
                    {step === 1 && (
                        <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                {!courierAlreadyAssigned && !alreadyHasShipment && shippingLocations.map((location) => (
                                    <div key={location.id}
                                        className={`border rounded-lg p-4 cursor-pointer transition-all ${selectedLocation?.id === location.id ? 'border-blue-500 bg-blue-50' : 'border-gray-200'}`}
                                        ...
                                        <div className="flex items-start justify-between">
                                            ...
                                            <input type="radio" name="pickup_location" checked={selectedLocation?.id === location.id} onChange={() => { if (!shipmentId) setSelectedLocation(location) }} />
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="mt-6 flex justify-end">
                                <button onClick={async () => { await fetchAvailableCouriers(); setStep(2) }} disabled={!selectedLocation || loading || shipmentId}
                    ...
                    {step === 2 && (
                        <div>
                            <div>
                                <h4 className="text-lg font-medium mb-4">Pickup Location</h4>
                            ...
                            {courierAlreadyAssigned ? (
                                ...
                                <div className="mt-6 flex justify-between items-center">
                                    <p className="text-green-700 text-sm">Shipment already created with ID: <strong>{shipmentId}</strong></p>
                                    <button
                                        onClick={() => shipmentCreated()}
                                        className="px-4 py-2 text-white bg-blue-600 rounded hover:bg-blue-700"
                                    >
                                        Next: Assign Courier
                                    </button>
                                ...
                                <div className="mt-6 flex justify-end gap-3">
                                    <button onClick={() => setStep(1)} className="px-4 py-2 bg-gray-100 text-gray-700 rounded">Back</button>
                                    <button
